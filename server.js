...
app.use(
  session({
    key: "kelimeapp_sid",
    secret: "kelimeapp_super_secret_123", // istersen değiştir
    store: sessionStore,
    resave: false,
...
function detectLang(word) {
  // Türkçe karakter varsa "tr" diyelim, yoksa "en"
...
// Admin - Kelime Düzenleme Sayfası (UPDATE ekranı)
...
// Search (korumalı istersen: requireAuth ekleyebilirsin)
  ...
  const sugText =
    suggestions.length
      ? "Bunu mu demek istedin: " +
        suggestions.map(s => `${s.tr} / ${s.en}`).join(", ")
